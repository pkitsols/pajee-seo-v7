...
const COMMON_FIELDS = {
  ...
  LodgingBusiness:['name','address'], Hotel:['name','address'], Resort:['name','address'], VacationRental:['name','address'],
  EducationalOrganization:['name','url'], School:['name','url'], CollegeOrUniversity:['name','url'], ProfessionalService:['name','address'],
  MedicalOrganization:['name','address'], Restaurant:['name','address'], Store:['name','address'], RealEstateAgent:['name','address'], TravelAgency:['name','address']
};
...
module.exports=async function schemaIntelligence(req,res){
  ...
  try{
    ...
    const scope=body.scope==='site'?'site':'page'; const maxPages=Math.min(200,Math.max(1,Number(body.maxPages)||50));
