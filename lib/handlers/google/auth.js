...
const { stateToken, cleanText } = require('../../api-lib');
...
function safeReturn(value) {
  const path = cleanText(value, '/tools/organic-visibility/');
  return path.startsWith('/') && !path.startsWith('//') ? path.slice(0, 1000) : '/tools/organic-visibility/';
}
...
module.exports = async function googleAuth(req, res) {
  try {
    ...
    const params = new URLSearchParams({
      ...
      state: stateToken({ returnTo: safeReturn(req.query?.return || req.query?.returnTo) })
    });
    res.statusCode = 302;
