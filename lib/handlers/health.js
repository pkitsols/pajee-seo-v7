...
module.exports = async function health(req, res) {
  return send(res, 200, {
    ...
    architecture: {
      ...
      oauthFunctions: ['/api/google/auth.js','/api/google/callback.js'],
      hobbyPlanCompatible: true
    },
    configured: {
      gemini: Boolean(process.env.GEMINI_API_KEY),
      geminiModel: String('gemini-3.5-flash').replace(/^models\//i, ''),
      backlinkGeminiAssist: !['0','false','off','no'].includes(String(process.env.BACKLINK_GEMINI_ASSIST || 'true').toLowerCase()),
