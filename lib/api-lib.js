...
async function gemini(prompt, { json = true } = {}) {
  ...
  const models = [process.env.GEMINI_MODEL, 'gemini-2.5-flash', 'gemini-2.5-flash-lite'].filter(Boolean).filter((m, i, a) => a.indexOf(m) === i);
  let lastError = 'Gemini request failed.';
  for (const model of models) {
    const url = `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${encodeURIComponent(key)}`;
    const body = { contents: [{ role: 'user', parts: [{ text: prompt }] }], generationConfig: { temperature: 0.18, maxOutputTokens: 8192 } };
    if (json) body.generationConfig.responseMimeType = 'application/json';
    try {
      const controller = new AbortController(); const timer = setTimeout(() => controller.abort(), 55000);
      let response; try { response = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body), signal: controller.signal }); } finally { clearTimeout(timer); }
      ...
      try { return JSON.parse(text); }
        ...
        const objectMatch = text.match(/\{[\s\S]*\}/); const arrayMatch = text.match(/\[[\s\S]*\]/);
        if (objectMatch) return JSON.parse(objectMatch[0]);
        if (arrayMatch) return JSON.parse(arrayMatch[0]);
        lastError = 'AI returned an unreadable response.';
      }
    } catch (error) { lastError = error.name === 'AbortError' ? 'The AI request timed out.' : error.message; }
  }
